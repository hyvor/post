<script lang="ts">
	...
	const mapKeys = [
		...
		'subscribed_at',
		'subscribe_ip',
		...$subscriberMetadataDefinitionStore.map((col) => `metadata_${col.key}`)
	] as const;
	...
	let showSelector = $state(Object.fromEntries(mapKeys.map((key) => [key, false])));
...
>
	<div class="modal-wrap">
		Select the columns from your CSV file that you want to map to the fields in our system. <strong
			>A mapping for the Email field is required</strong
		>. You may leave other fields unmapped if you don't want to import them.
				...
				<ActionList>
					<div class="action-list">
						{#each filteredFields as filteredColumn}
							<ActionListItem on:click={() => handleSelect('email', filteredColumn)}>
								{filteredColumn}
							</ActionListItem>
						{/each}
					</div>
		...
		<SplitControl label="Lists">
			...
			>
				...
				<ActionList>
					<div class="action-list">
						{#each filteredFields as filteredColumn}
							<ActionListItem on:click={() => handleSelect('lists', filteredColumn)}>
								{filteredColumn}
							</ActionListItem>
						{/each}
					</div>
