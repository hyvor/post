<script lang="ts">
    ...
    let {importItem, showFieldsOf, showFieldMappingModalOf, showWarningsOf, importDisabled = false}: Props = $props();
...
<button
        class={`import-item ${
			importItem.status !== 'requires_input' || importDisabled
			? 'import-item-disabled'
			: ''
		}`}
        onclick={() => !importDisabled && importItem.status === 'requires_input' && showFieldMappingModalOf(importItem)}
>
