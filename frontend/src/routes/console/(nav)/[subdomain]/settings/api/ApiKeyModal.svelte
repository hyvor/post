<script lang="ts">
	...
	function handleSubmit() {
		...
		promise
				...
				if (editingApiKey) {
					...
					toast.success(
						I18n.t('console.common.updated', {
							field: I18n.t('console.settings.api.apiKey')
						})
					);
				} else {
			...
			.catch((error) => {
				console.error(
					I18n.t('console.settings.api.failedAction', {
						field: editingApiKey
							? I18n.t('console.settings.api.update')
							: I18n.t('console.settings.api.create')
					}),
					error
				);
				toast.error(
	...
	const modalTitle = $derived(
		isEditing
			? I18n.t('console.common.editField', { field: I18n.t('console.settings.api.apiKey') })
			: I18n.t('console.common.createField', { field: I18n.t('console.settings.api.apiKey') })
	);
	const confirmText = $derived(
		isEditing
			? I18n.t('console.common.updateField', { field: I18n.t('console.settings.api.apiKey') })
			: I18n.t('console.common.createField', { field: I18n.t('console.settings.api.apiKey') })
	);
</script>
...
>
	<div class="modal-content">
		...
		{#if isEditing}
			<SplitControl
				label={I18n.t('console.settings.api.status')}
				caption={I18n.t('console.settings.api.statusCaption')}
			>
				<Switch bind:checked={isEnabled} disabled={loading}>
					{isEnabled
						? I18n.t('console.settings.api.enabled')
						: I18n.t('console.settings.api.disabled')}
				</Switch>
			</SplitControl>
		{/if}
	</div>
