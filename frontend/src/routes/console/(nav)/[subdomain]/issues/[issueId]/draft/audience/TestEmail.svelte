<script lang="ts">
	...
	function handleConfirm() {
		...
		sendIssueTest($draftIssueEditingStore.id, selectedEmails)
			.then((res) => {
				...
				if (count > 1) {
					toast.success(
						I18n.t('console.issues.draft.testEmail.confirm.sentMultiple', {
							count: count
						}),
						{ id: toastId }
					);
				} else if (count === 1) {
					toast.success(I18n.t('console.issues.draft.testEmail.confirm.sent'), {
						id: toastId
					});
				} else {
					toast.error(I18n.t('console.issues.draft.testEmail.confirm.failed'), {
						id: toastId
					});
				}
	...
	onMount(() => {
		getIssueTestData($draftIssueEditingStore.id).then((res) => {
...
<div class="wrap">
	...
	<Button color="input" onclick={() => (showTestEmailModal = true)}>
		...
		<div class="suggested-emails">
			{#if suggestedEmails.every((email) => selectedEmails.includes(email))}
				<div class="no-suggestions">
					{I18n.t('console.issues.draft.testEmail.modal.noSuggested')}
				</div>
			{:else}
