...
<a class="issue" href={consoleUrlWithNewsletter(`/issues/${issue.id}`)}>
	...
	<div class="results">
		{#if issue.status === 'sent'}
			<SentStat value={issue.total_sends.toLocaleString()} name="Sent" />
			<SentStat value={percentage(issue.opened_sends, issue.total_sends)} name="Opened" />
			<SentStat value={percentage(issue.clicked_sends, issue.total_sends)} name="Clicked" />
		{/if}
	</div>
</a>
