...
function getSudoApi() {
  ...
  return {
    call,
    get: async <T>(opt: ConsoleApiOptions) =>
      call<T>({ ...opt, method: "get" }),
    post: async <T>(opt: ConsoleApiOptions) =>
      call<T>({ ...opt, method: "post" }),
    patch: async <T>(opt: ConsoleApiOptions) =>
      call<T>({ ...opt, method: "patch" }),
    put: async <T>(opt: ConsoleApiOptions) =>
      call<T>({ ...opt, method: "put" }),
    delete: async <T>(opt: ConsoleApiOptions) =>
      call<T>({ ...opt, method: "delete" }),
  };
}
