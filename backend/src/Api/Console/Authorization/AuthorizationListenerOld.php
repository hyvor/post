...
class AuthorizationListenerOld
{
    ...
    public function __invoke(ControllerEvent $event): void
    {
        ...
        if ($authorizationHeader) {
            ...
            if ($this->isProductApiKey($bearerToken)) {
                $this->handleProductApiKey($event, $bearerToken);
            } else {
                $this->handleCloudToken($event, $bearerToken);
            }
        } else {
    ...
    /**
     * if the length is 32 and it is a hex string, then it is a product API key
     * otherwise, we can check for JWT cloud token
     */
    private function isProductApiKey(string $apiKey): bool
    {
        return strlen($apiKey) === ApiKeyService::API_KEY_LENGTH && ctype_xdigit($apiKey);
    }

    private function handleProductApiKey(ControllerEvent $event, string $apiKey): void
    {
        $request = $event->getRequest();
    ...
    private function handleCloudToken(ControllerEvent $event, string $possibleJwtToken): void
    {
        $request = $event->getRequest();
        ...
        $cloudToken = $this->cloudApiService->decodeJwtToken($possibleJwtToken);

        if ($noOrganizationRequired) {
            throw new AccessDeniedHttpException(
                'OrganizationOptional endpoints are not supported with Cloud API tokens.',
            );
        }

        $this->verifyScopes($cloudToken->getScopesFor(Component::POST), $event);
        ...
        if ($isOrganizationLevelEndpoint === false) {
            ...
            if ($newsletter->getOrganizationId() !== $cloudToken->orgId) {
